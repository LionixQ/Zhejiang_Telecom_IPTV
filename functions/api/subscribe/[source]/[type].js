 ...
 *
 * Query params:
 *   udpxy   — (required for multicast) UDPXY address (private IP:PORT only)
 *   fcc     — (optional, multicast only) FCC server address (IP:PORT only)
...
export async function onRequestGet(context) {
  ...
  if (source !== 'multicast' && source !== 'unicast') {
    return jsonError(400, '参数无效，仅支持 /api/subscribe/multicast 或 /api/subscribe/unicast');
  }

  // ── Validate type ────────────────────────────────────
  if (type !== 'm3u' && type !== 'txt') {
    return jsonError(400, '参数无效，仅支持 /api/subscribe/{source}/m3u 或 /api/subscribe/{source}/txt');
  }

  // ── Validate udpxy ───────────────────────────────────
  if (source === 'multicast' && !udpxy) {
    return jsonError(400, '组播的udpxy参数为必填');
  }
  if (source === 'unicast' && udpxy) {
    return jsonError(400, 'udpxy参数仅在 source=multicast 时有效');
  }
  if (udpxy && !isValidUDPXY(udpxy)) {
    return jsonError(400, 'udpxy参数格式无效，应为内网 IP:端口（如 192.168.x.x:2340）');
  }

  // ── Validate fcc ─────────────────────────────────────
  if (source === 'unicast' && fcc) {
    return jsonError(400, 'fcc 参数仅在 source=multicast 时有效');
  }
  if (fcc && !isValidIPPort(fcc)) {
    return jsonError(400, 'fcc 参数格式无效，应为 IP:端口（如 10.255.14.152:15970）');
  }
  ...
  if (logoParam && !isValidHost(logoParam)) {
    return jsonError(400, 'logo 参数格式无效，应为 IP或域名，可带端口');
  }
  ...
  // ── Enable FCC for multicast stream URLs ─────────────
  if (source === 'multicast' && fcc) {
    content = appendQueryParamToStreamURLs(content, 'fcc', fcc);
  }
...
/** Append a query parameter to HTTP(S) stream URL lines in M3U/TXT content. */
function appendQueryParamToStreamURLs(content, name, value) {
  return content.split(/(\r?\n)/).map((line) => {
    const urlMatch = line.match(/^(.*?,)?(https?:\/\/.*)$/i);
    if (!urlMatch) return line;

    const prefix = urlMatch[1] || '';
    const streamURL = urlMatch[2];
    const hashIndex = streamURL.indexOf('#');
    const base = hashIndex >= 0 ? streamURL.substring(0, hashIndex) : streamURL;
    const fragment = hashIndex >= 0 ? streamURL.substring(hashIndex) : '';
    const separator = base.includes('?') ? '&' : '?';
    return `${prefix}${base}${separator}${name}=${value}${fragment}`;
  }).join('');
}

/** Validate strict IPv4:PORT, with port range 1-65535. */
function isValidIPPort(s) {
  ...
  return portNum >= 1 && portNum <= 65535 && isValidIPv4(host);
}
...
/** Validate "IP:PORT" only — udpxy does not resolve domain names */
function isValidUDPXY(s) {
  const idx = s.lastIndexOf(':');
  if (idx <= 0) return false;
  const host = s.substring(0, idx);
  const port = s.substring(idx + 1);
  ...
  // Must be IPv4 (each octet 0-255, no leading zeros)
  const ipv4Re = /^(\d{1,3}\.){3}\d{1,3}$/;
  if (!ipv4Re.test(host)) return false;
  ...
  // Must be a private IP (10.x, 172.16-31.x, 192.168.x)
